...
void main(int argc,char **argv)
{
	...
	while(read_line(stdin)) {
		...
		blh.lat	=deg_to_rad(atof(get_field(0)));/* 緯度[deg] */
		blh.lon	=deg_to_rad(atof(get_field(0)));/* 経度[deg] */
		blh.hgt	=atof(get_field(0));			/* 高度[m] */

		/* 座標変換して出力する */
		printf("%.3f,%.4f,%.4f,%.4f,%s\n",
			sec,								/* 時刻[s] */
			blh_to_xyz(blh).x,					/* X座標[m] */
			blh_to_xyz(blh).y,					/* Y座標[m] */
			blh_to_xyz(blh).z,					/* Z座標[m] */
