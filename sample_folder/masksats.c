/*------------------------------------------------------------
 * MASKSATS.c - Mask Angle versus the Number of Satellites.
 *------------------------------------------------------------*/
...
#define	MASK_STOP			50.0
#define	MASK_STEP			1.0
...
{
	int		n,prn;
	long	epochs,avail,sats;
	double	sec,mask,start,stop,step;
	FILE	*fp;
	wtime	wt;
	posxyz	satpos,usrpos;
	posblh	user;

	/* RINEX航法ファイルを読み込む */
	if (argc<8) {
		fprintf(stderr,"masksats <RINEX-NAV> <lat> <lon> <hgt> \
<start> <stop> <step>\n");
		exit(0);
	...
	/* ユーザ位置 */
	user.lat=deg_to_rad(atof(argv[2]));
	user.lon=deg_to_rad(atof(argv[3]));
	user.hgt=atof(argv[4]);
	usrpos	=blh_to_xyz(user);
	...
	/* 仰角マスクを変えながら計算する */
	for(mask=MASK_START;mask-MASK_STOP<0.001;mask+=MASK_STEP) {
		epochs	=0;
		sats	=0;
		avail	=0;

		/* 指定された時間範囲について計算 */
		for(sec=start;sec-stop<0.001;sec+=step) {
			wt.week	=current_week;
			wt.sec	=sec;
			...
			for(prn=1;prn<=MAX_PRN;prn++) {
				/* エフェメリスをセット */
				if (!set_ephemeris(prn,wt,-1)) continue;

				/* 衛星位置を計算 */
				satpos	=satellite_position(prn,wt,0.0);

				/* 仰角マスクによる選別 */
				if (elevation(satpos,usrpos)>deg_to_rad(mask)) n++;
			}
			epochs++;
			if (n>=4) avail++;
			sats+=n;
		...
		printf("%.2f,%d,%.6f,%.6f\n",
			mask,					/* #1: 仰角マスク[deg] */
			epochs,					/* #2: エポック数 */
			(double)sats/epochs,	/* #3: 平均衛星数 */
			(double)avail/epochs);	/* #4: 測位可能エポックの割合 */
	}
