/*------------------------------------------------------------
 * XYZ2ENU.c - Coordinate Conversion.
 *------------------------------------------------------------*/
...
void main(int argc,char **argv)
{
	...
	/* コマンドライン引数をチェック */
	if (argc!=4) {
		printf("xyz2enu - convert XYZ into ENU.\n");
		printf("\n");
		printf("usage: xyz2enu <lat> <lon> <hgt> <in >out\n");
		printf("\n");
		exit(0);
	}

	/* 基準位置 */
	blh.lat	=deg_to_rad(atof(argv[1]));
	blh.lon	=deg_to_rad(atof(argv[2]));
	blh.hgt	=atof(argv[3]);
	rel_pos	=blh_to_xyz(blh);

	/* データファイルを読み込む */
	while(read_line(stdin)) {
		sec		=atof(get_field(0));			/* 時刻 */
		pos.x	=atof(get_field(0));			/* X座標[m] */
		pos.y	=atof(get_field(0));			/* Y座標[m] */
		pos.z	=atof(get_field(0));			/* Z座標[m] */

		/* 座標変換して出力する */
		printf("%.3f,%.4f,%.4f,%.4f,%s\n",
			sec,								/* 時刻[s] */
			xyz_to_enu(pos,rel_pos).e,			/* E座標[m] */
			xyz_to_enu(pos,rel_pos).n,			/* N座標[m] */
			xyz_to_enu(pos,rel_pos).u,			/* U座標[m] */
			get_field(LINEBUF_LEN));			/* 残り */
	}
