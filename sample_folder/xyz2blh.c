/*------------------------------------------------------------
 * XYZ2BLH.c - Coordinate Conversion.
 *------------------------------------------------------------*/
...
void main(int argc,char **argv)
{
	...
	/* コマンドライン引数をチェック */
	if (argc!=1) {
		printf("xyz2blh - convert XYZ into LAT/LON/HGT.\n");
		printf("\n");
		printf("usage: xyz2blh <in >out\n");
		printf("\n");
		exit(0);
	}
	...
	while(read_line(stdin)) {
		...
		/* 座標変換して出力する */
		printf("%.3f,%.9f,%.9f,%.4f,%s\n",
			sec,								/* 時刻[s] */
			rad_to_deg(xyz_to_blh(pos).lat),	/* 緯度[deg] */
			rad_to_deg(xyz_to_blh(pos).lon),	/* 経度[deg] */
			xyz_to_blh(pos).hgt,				/* 高度[m] */
